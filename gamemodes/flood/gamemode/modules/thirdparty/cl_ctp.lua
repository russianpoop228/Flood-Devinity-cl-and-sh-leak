--[[
CTP made by CapsAdmin
http://steamcommunity.com/sharedfiles/filedetails/?id=108168164
]]

AddSettingsItem("thirdperson", "button", "", {lbl = "Toggle Thirdperson", func = function() RunConsoleCommand("ctp") end})
AddSettingsItem("thirdperson", "button", "", {lbl = "Open Thirdperson Menu", func = function() RunConsoleCommand("ctp_toggle_menu") end})
...
do -- Enable
	...
	function ctp:SetAllowed(allowed)
		self = self or ctp

		self.Allowed = allowed

		if not allowed and self.Enabled then
			self.WasEnabled = true
			self:Disable()
		elseif allowed and self.WasEnabled then
			self.WasEnabled = nil
			self:Enable()
		end
	end

	hook.Add("FMIronsightsChange", "CTP", function(enabled)
		if IsFirstTimePredicted() then
			ctp:SetAllowed(not enabled)
		end
	end)
