...
function TOOL:LeftClick( trace, attach )
	...
	if ( IsValid( trace.Entity ) && trace.Entity:GetClass() == "gmod_light_fm" && trace.Entity:GetPlayer() == ply ) then
...
if ( SERVER ) then
	...
	function MakeLight( pl, r, g, b, brght, size, toggle, on, KeyDown, Data )
		...
		local lamp = ents.Create( "gmod_light_fm" )
		...
		if ( IsValid( pl ) ) then
			...
			LogFile({"{ply1} spawned a light", pl}, "prop")
		end

		return lamp

	end
	duplicator.RegisterEntityClass( "gmod_light_fm", MakeLight, "lightr", "lightg", "lightb", "Brightness", "Size", "Toggle", "on", "KeyDown", "Data" )
...
function TOOL:UpdateGhostLight( ent, pl )
	...
	if ( !trace.Hit || IsValid( trace.Entity ) && ( trace.Entity:IsPlayer() || trace.Entity:GetClass() == "gmod_light_fm" ) ) then
