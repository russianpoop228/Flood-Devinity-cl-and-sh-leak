...
local function SetColour( ply, ent, data )
	...
	if ( data.Color ) then ent:SetColor( Color( data.Color.r, data.Color.g, data.Color.b ) ) end
...
function TOOL:LeftClick( trace )
	...
	SetColour( self:GetOwner(), ent, { Color = Color( r, g, b ), RenderMode = mode, RenderFX = fx } )
	return true
...
function TOOL.BuildCPanel( CPanel )
	...
	local ctrl = CPanel:AddControl( "Color", { Label = "#tool.colour.color", Red = "colour_r", Green = "colour_g", Blue = "colour_b", Alpha = "colour_a" } )
	ctrl.Mixer.txtA:SetVisible(false)
	ctrl.Mixer.Alpha:SetVisible(false)
end
