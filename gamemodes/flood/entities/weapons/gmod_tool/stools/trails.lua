...
local function SetTrails( ply, ent, data )
	...
	local s = math.min(maxsize, data.StartSize)
	local e = math.min(maxsize, data.EndSize)
	local l = math.min(maxlength, data.Length)

	local trail_entity = util.SpriteTrail( ent, 0, data.Color, false, s, e, l, 1 / ( ( s + e ) * 0.5 ), data.Material .. ".vmt" )
...
function TOOL:LeftClick( trace )
	...
	if ( !game.SinglePlayer() ) then
		...
		length = math.Clamp( length, 0.1, maxlength )
		endsize = math.Clamp( endsize, 0, maxsize )
		startsize = math.Clamp( startsize, 0, maxsize )
...
function TOOL.BuildCPanel( CPanel )
	...
	CPanel:NumSlider( "#tool.trails.length", "trails_length", 0, maxlength, 2 )
	CPanel:NumSlider( "#tool.trails.startsize", "trails_startsize", 0, maxsize, 2 )
	CPanel:NumSlider( "#tool.trails.endsize", "trails_endsize", 0, maxsize, 2 )
