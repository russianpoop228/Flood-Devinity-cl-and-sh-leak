...
local function DoRemoveEntity( ply, ent )
    ...
    if hook.Run("PlayerRemoveEnt", ply, ent) == false then return false end
    ...
    hook.Call("PlayerRemovedEnt", nil, ply, ent)
...
function TOOL:LeftClick( trace )
    ...
    if ( DoRemoveEntity( self:GetOwner(), trace.Entity ) ) then
...
function TOOL:RightClick( trace )
    ...
    for _, Entity in pairs( ConstrainedEntities ) do
        ...
        if ( DoRemoveEntity( self:GetOwner(), Entity ) ) then
            Count = Count + 1
        end
