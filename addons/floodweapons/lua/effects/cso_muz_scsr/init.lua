...
function EFFECT:Init(data)
	...
	local sparticle = emitter:Add("tracers/muzzle_star_in", self.vOffset)
	...
	if (sparticle) then
		...
		sparticle:SetStartSize(80 * (halofac * 0.8 + 0.2), 0, 1)
	...
	for i = 1, 1 do
		local particle = emitter:Add("tracers/muzzle_star_in", self.vOffset)
	...
	for i = 0, 5 do
		local particle = emitter:Add("tracers/muzzle_star", self.vOffset + (dir * 0.6 * i))
		...
		if (particle) then
			...
			--particle:SetEndSize( 0 )
			particle:SetStartSize(math.max(18 - 0.65 * i, 1) * 0.55)
	...
	for i = 0, sparkcount do
		local particle = emitter:Add("tracers/muzzle_star_in", self.Position)
