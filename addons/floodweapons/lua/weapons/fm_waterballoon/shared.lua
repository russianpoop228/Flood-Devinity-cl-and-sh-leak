...
if (CLIENT) then --the cl_init.lua stuff goes in here
   ...
   SWEP.DrawCrosshair = false;
	SWEP.WepSelectIcon = surface.GetTextureID("vgui/hud/fm_waterballoon")
	SWEP.BounceWeaponIcon = false
	SWEP.DrawWeaponInfoBox = false		-- Should draw the weapon info box
end

SWEP.Instructions = "Left click throws water balloons, these extinguish fires. Right click to throw at yourself. Reload to pick a new balloon color.";
...
function SWEP:Deploy()
	local Generation = HSVToColor(math.random(0,255),1,1) 
	...
	if self.Weapon:Clip1() > 0 then
		...
		self:SetClip1(self:Clip1() - 1)
			if SERVER then
				...
				balloonphys:Wake()
				balloonphys:ApplyForceCenter(self.Owner:GetAimVector():GetNormalized() * 3000 + self.Owner:GetVelocity() * 2)
				balloonphys:SetBuoyancyRatio( 0.03 )
				self.Owner:SetAnimation( PLAYER_ATTACK1 )
				self.Weapon:SendWeaponAnim( ACT_VM_THROW )
				timer.Simple(0.5, function() if not IsValid(self) or not IsValid(self.Owner) then return end local Generation = HSVToColor(math.random(0,255),1,1) self.Weapon:SetNWVector("MyColor",Vector(Generation.r,Generation.g,Generation.b)) self.Weapon:SendWeaponAnim(ACT_VM_DRAW) end)
				if self.Weapon:Clip1() <= 0 then
...
function SWEP:SecondaryAttack()
	if self.Weapon:Clip1() > 0 and self.Weapon:GetNextPrimaryFire() <= CurTime() then
		self.Weapon:SetNextPrimaryFire(CurTime() + self.Primary.Delay)
		self:SetClip1(self:Clip1() - 1)
			if SERVER then
				local Received = self.Weapon:GetNWVector("MyColor",Vector( 255, 255, 255))
				local balloon = ents.Create("ent_wballoon") 
				balloon:SetPos(self.Owner:EyePos()) 
				balloon:SetAngles(Angle(math.random(0, 360),math.random(0, 360),0))
				balloon:SetColor( Color(Received.x, Received.y, Received.z, 255) )
				balloon:Spawn()
				local balloonphys = balloon:GetPhysicsObject()
				balloonphys:Wake()
				balloonphys:ApplyForceCenter(Vector(0, 0, -1000) + self.Owner:GetVelocity() * 2)
				balloonphys:SetBuoyancyRatio( 0.03 )
				self.Owner:SetAnimation( PLAYER_ATTACK1 )
				self.Weapon:SendWeaponAnim( ACT_VM_THROW )
				timer.Simple(0.5, function() if not IsValid(self) or not IsValid(self.Owner) then return end local Generation = HSVToColor(math.random(0,255),1,1) self.Weapon:SetNWVector("MyColor",Vector(Generation.r,Generation.g,Generation.b)) self.Weapon:SendWeaponAnim(ACT_VM_DRAW) end)
