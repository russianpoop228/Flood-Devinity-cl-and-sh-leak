...
SWEP.Author				= "Upset" --Author Tooltip
...
SWEP.PrintName				= "RPG\n(ROCKET PROPELLED GRENADE)"		-- Weapon name (Shown on HUD)
...
--[[WEAPON HANDLING]]--
SWEP.Primary.Sound = Sound("TFA_MMOD.RPG.1") -- This is the sound of the weapon, when you shoot.
...
SWEP.Primary.ProjectileVelocity = 11614.2 * 16 / 12-- 295 meters to inches * 4/3 ( 16 / 12 hammer to inches )
...
SWEP.Primary.DefaultClip = 4 -- This is the number of bullets the gun gives you, counting a clip as defined directly above.
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_mmod/c_rpg.mdl" --Viewmodel path
...
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/w_rocket_launcher.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(-2.76, -4.795, -1.112)
SWEP.IronSightsAng = Vector(2.742, -0.352, 7.368)
SWEP.IronSightTime = 0.75
--[[INSPECTION]]--
SWEP.InspectPos = Vector(5, -5.619, -2.787)
SWEP.InspectAng = Vector(10.386, 34.417, 5)
...
function SWEP:ShootBulletInformation()
	...
	if SERVER then
		local pRocket = ents.Create("mmod_rpg_missile")
...
function SWEP:UpdateSpot()
	if !IsValid(self:GetSpotEntity()) then
		if SERVER then self:SetSpotEntity(ents.Create("mmod_laserdot")) end
		local spotEnt = self:GetSpotEntity()
		if IsValid(spotEnt) then
			spotEnt:SetPos(self.Owner:GetEyeTrace().HitPos)
